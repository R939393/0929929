...
// Menyimpan Log Chat ke File
const logFilePath = path.join(__dirname, './HANZ-DATA/bot_chat.log');
const rawLogText = `[CHAT] (${locale_day} ${date} - ${date_time}) | ID: ${m.key.id} | MSG: ${budy || m.type} | FROM: ${m.pushName || (isCreator ? 'Bot' : 'Anonim')} (${m.sender.split('@')[0]}) | CHAT: ${m.isGroup ? m.metadata?.subject || 'Group' : m.chat.endsWith('@newsletter') ? 'Newsletter' : 'Private Chat'} (${m.chat})\n`;
fs.appendFileSync(logFilePath, rawLogText);
...
case 'add': {
    ...
    if (!m.isAdmin) return m.reply('Fitur ini khusus Admin Grup!');
    if (!m.isBotAdmin) return m.reply('Bot harus menjadi Admin Grup terlebih dahulu!');
...
//////////////////////////////////     HANZ    ////////////////////////////////////
// ==================== FITUR SIMPAN MEDIA ====================
// ==================== FITUR SIMPAN MEDIA (FIXED UNTUK APLIKASI/DOKUMEN) ====================
case 'sv': {
    if (!isCreator) return m.reply('Khusus Owner!');
    if (!text) return m.reply(`Masukkan nama command!\nContoh: *${prefix + command} apkbot*`);
    ...
    try {
        ...
        m.reply(`✅ Media berhasil disimpan!\n\n◦ *Nama:* ${cmdKey}\n◦ *Tipe:* ${mediaType}\n\nUntuk memanggil gunakan: *${prefix}kirim ${cmdKey}*`);
    } catch (err) {
        console.error(err);
        m.reply('❌ Gagal mengunduh dan menyimpan media. Pastikan bot merespon media yang benar.');
    }
}
break;
...
case 'list03': {
    ...
    teks += `_Gunakan *${prefix}kirim <nama>* untuk mengirim media._\n`;
    teks += `_Gunakan *${prefix}hapus <nama>* untuk menghapus media._`;
...
case 'getlog': {
    ...
    if (!fs.existsSync(logFilePath)) return m.reply('File log belum tersedia!');

    await sendLoading(m.chat, m);
    await RAEHAN2GD.sendMessage(m.chat, {
        document: fs.readFileSync(logFilePath),
        mimetype: 'text/plain',
        fileName: `Console_Log_${date.replace(/\//g, '-')}.log`,
        caption: `📄 *FILE LOG BOT CONSOLE*\n\n🕒 Ditransfer pada: ${date_time}`
    }, { quoted: fkontak });
}
break;
