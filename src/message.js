...
async function GroupParticipantsUpdate(RAEHAN2GD, update, store) {
    try {
        ...
        let groupMetadata;
try {
    groupMetadata = await RAEHAN2GD.groupMetadata(id);
    if (store && store.groupMetadata) {
        store.groupMetadata[id] = groupMetadata;
    }
} catch {
    groupMetadata = store?.groupMetadata?.[id] || { subject: 'Group', participants: [] };
}
        const groupName = groupMetadata.subject || 'Group';
        const memberCount = (groupMetadata.participants || []).length.toString();
        ...
        for (let participant of participants) {
            const num = typeof participant === 'string' ? participant : (participant?.id || participant?.phoneNumber || '');
            if (!num) continue;
            ...
            if (action === 'add' || action === 'remove') {
                let ppUser;
                try {
                    ppUser = await RAEHAN2GD.profilePictureUrl(num, 'image', 3000);
                } catch {
                    ppUser = 'https://i.ibb.co/G5mJZxs/noprofile.jpg';
                }

                const isAdd = action === 'add';
                
                // 1. Masukkan Direct Link (URL) gambar mentahan Anda di sini
                const backgroundUrl = isAdd 
                    ? 'https://k.top4top.io/p_39060sw9j0.jpg' 
                    : 'https://e.top4top.io/p_3906stnbk0.jpg';

                const captionText = isAdd ?
...
𝐀𝐋𝐋 𝐌𝐄𝐌𝐁𝐄𝐑
 ${groupName}
┏━━⊱
┣❏BUY SCRIPT : https://wa.me/6282299938030
┗━━⊱
▬▭▬▭▬▭▬▭▬▬▭▬▭▬`;

                try {
                    // 2. Membaca background langsung dari URL (tidak butuh file lokal)
                    ...
                    try {
                        ...
                        pp = await Jimp.read('https://i.ibb.co/G5mJZxs/noprofile.jpg'); 
                    }

                    // 3. Menurunkan foto profil agar pas di tengah (Y diubah menjadi 225)
                    pp.resize({ w: 130, h: 130 }); 
                    bg.composite(pp, 45, 216); 

                    const font = await loadFont(SANS_32_BLACK);

                    bg.print({ font, x: 425, y: 195, text: groupName }); 
                    bg.print({ font, x: 425, y: 260, text: jamAktif });  
                    bg.print({ font, x: 425, y: 325, text: memberCount }); 

                    const buffer = await bg.getBuffer('image/jpeg');
                    
                    await RAEHAN2GD.sendMessage(id, { 
                        image: buffer, 
                        caption: captionText,
                        mentions: [num] 
                    });
                    ...
                    console.error('[ERROR] Gagal memproses gambar welcome/goodbye:', err);
                }
            }
        }
    } catch (e) {
